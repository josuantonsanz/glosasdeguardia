...
def is_home_note(post):
    """Returns True if the note is designated as the site's home page (dg-home: true)."""
    home = post.metadata.get("dg-home", False)
    return home is True or str(home).lower() == "true"
...
def extract_metadata(content, frontmatter_dict, fallback_title, extracted_images=None):
    ...
    # "created"/"planted", normalized to a plain YYYY-MM-DD (the sitemap
    # protocol requires W3C datetime format).
    lastmod_source = updated_val or created_val
    lastmod_match = re.match(r'(\d{4}-\d{2}-\d{2})', str(lastmod_source)) if lastmod_source else None
    metadata["lastmod"] = lastmod_match.group(1) if lastmod_match else None
...
def build_site():
    ...
    # Guard against URL collisions: several notes can map to the same output URL
    # (e.g. a stray content/index.md vs. the dg-home note, which both become
    # "index.html"). The home note wins; the duplicate is dropped so the output
    # and the sitemap don't contain two pages with the same URL.
    notes.sort(key=lambda np: 0 if is_home_note(np[1]) else 1)
    seen_urls = set()
    filtered_notes = []
    for filepath, post in notes:
        url = "index.html" if is_home_note(post) else filepath.relative_to(Path(CONTENT_DIR)).with_suffix('.html').as_posix()
        if url in seen_urls:
            print(f"WARNING: Skipping {filepath} — URL '{url}' is already generated by another note.")
            continue
        seen_urls.add(url)
        filtered_notes.append((filepath, post))
    ...
    # Build the sitemap with unique, absolute, percent-encoded URLs. Raw spaces,
    # parentheses, '&', accents, or emoji are not valid in <loc> values.
    # The root URL (home) is emitted by the template itself, so it is pre-seeded.
    sitemap_notes = []
    seen_locs = {f"{SITE_URL}/"}
    for note in all_notes_data:
        if note["url"] == "index.html":
            loc = f"{SITE_URL}/"
        else:
            loc = f"{SITE_URL}/{quote(note['url'], safe='/')}"
        if loc in seen_locs:
            continue
        seen_locs.add(loc)
        sitemap_note = dict(note)
        sitemap_note["loc"] = loc
        sitemap_notes.append(sitemap_note)
    sitemap_xml = sitemap_template.render(notes=sitemap_notes, site_url=SITE_URL)
